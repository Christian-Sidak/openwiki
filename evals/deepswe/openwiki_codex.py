...
class OpenWikiCodex(BaselineCodex):
    ...
    async def run(
        ...
        treatment_instruction = (
            "OpenWiki treatment condition: follow the OpenWiki instructions installed "
            "in the repository's AGENTS.md. Start with the generated quickstart at "
            f"{quickstart_path} and use the read-only openwiki_retrieval MCP tools as "
            "needed. The wiki was generated from the same base checkout: treat /app as "
            "the source of truth, make all code changes only in /app, and do not edit "
            "/tmp/openwiki-source.\n\n"
            f"{instruction}"
        )
